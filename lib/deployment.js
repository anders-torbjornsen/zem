...
class Deployment {
    ...
    currentDeployment) {
        ...
        const deployment = await this._deploy(artifact, contractConfig.linkTable, await getArgs());
    ...
    async _deployImpl(contractConfig, currentAddress) {
        ...
        currentDeployment = await this._deploy(artifact, contractConfig.linkTable, []);
    ...
    async _deploy(artifact, linkTable, args) {
        const fullyQualifiedContractName = getFullyQualifiedContractName(artifact);
        ...
        const contractFactory = new ethers_1.ContractFactory(artifact.abi, bytecode, this._signer);
...
function getFullyQualifiedContractName(artifact) {
    return `${artifact.sourceName}:${artifact.contractName}`;
}
function getBytecodeHash(artifact, linkTable) {
...
function linkBytecode(bytecode, linkReferences, linkTable) {
