...
exports.FacetCutAction = exports.getFacets = exports.Deployment = void 0;
...
class Deployment {
    ...
    async deploy(id, contractConfig, ...args) {
        ...
        this._deployment.contracts[id] = await this._deployContract(contractConfig, args, this._deployment.contracts[id]);
    ...
    async deployERC1967(id, contractConfig, upgradeFunc, getProxyConstructorArgs) {
        ...
        this._deployment.contracts[id] = await this._deployContract(contractConfig.proxy, getProxyConstructorArgs ? getProxyConstructorArgs(implementation) : [], contractDeployment);
    ...
    async deployDiamond(id, contractConfig, getProxyConstructorArgs) {
        ...
        this._deployment.contracts[id] = await this._deployContract(contractConfig, proxyConstructorArgs, this._deployment.contracts[id]);
    ...
    async _deployContract(contractConfig, args, currentDeployment) {
    ...
    calculateDiamondCut(facets, currentFacets) {
        ...
        for (const facet of getFacets(facets, this._hre)) {
            ...
            for (const selector of facet.selectors) {
                selectorToAddress[(0, bytes_1.hexlify)(selector)] = facetAddress;
            }
        }
...
function getFacets(facets, hre) {
    ...
    for (const facetConfig of facets) {
        ...
        const facetInterface = new abi_1.Interface(artifact.abi);
...
(function (FacetCutAction) {
    ...
    FacetCutAction[FacetCutAction["Update"] = 1] = "Update";
