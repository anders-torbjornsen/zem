...
class Deployment {
    ...
    async deployERC1967(contractConfig, getProxyConstructorArgs, upgradeFunc) {
        ...
        else if (this._deployedContracts.contracts[contractConfig.id]
        ...
        // proxies
        let currentImplementation = await this._hre.ethers.provider.getStorageAt(proxy.address, "0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc");
        ...
        try {
            ...
            for (const contractId in this._deployedContracts.contracts) {
                ...
                if (this._deployedContracts.contracts[contractId]
                    .implementation != undefined) {
                    usedBuildInfoIds.add(deployedContract.implementation.buildInfo);
                }
            }
            let toPrune = [];
